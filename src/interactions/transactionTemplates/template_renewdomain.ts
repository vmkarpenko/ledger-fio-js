import type {HexString, ParsedActionAuthorisation, ParsedMapBlockchainPublicAddress, ParsedRemoveAllMappedAddresses, ParsedRenewDomain, ParsedTransaction, ParsedTransferFIOTokensData, Uint8_t, ValidBIP32Path} from "../../types/internal"
...
const CONTRACT_NAME = parseNameString("renewdomain", InvalidDataReason.UNEXPECTED_ERROR);


export function template_renewdomain(chainId: HexString, tx: ParsedTransaction, parsedPath: ValidBIP32Path): Array<Command> {
    ...
    const actionData: ParsedRenewDomain = tx.actions[0].data as ParsedRenewDomain;
