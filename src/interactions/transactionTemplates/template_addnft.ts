import type {HexString, ParsedActionAuthorisation, ParsedMapNFTSignature, ParsedTransaction, Uint8_t, ValidBIP32Path} from "../../types/internal"
...
function template1(chainId: HexString, tx: ParsedTransaction, parsedPath: ValidBIP32Path): Array<Command> {
    ...
    return [
        ...
        COMMAND_APPEND_DATA_STRING_WITH_LENGTH_DO_NOT_SHOW(Buffer.from(actionData.nfts[0].hash), 0, 64),
...
function template2(chainId: HexString, tx: ParsedTransaction, parsedPath: ValidBIP32Path): Array<Command> {
    ...
    return [
        ...
        COMMAND_APPEND_DATA_STRING_WITH_LENGTH_DO_NOT_SHOW(Buffer.from(actionData.nfts[1].hash), 0, 64),
...
function template3(chainId: HexString, tx: ParsedTransaction, parsedPath: ValidBIP32Path): Array<Command> {
    ...
    return [
        ...
        COMMAND_APPEND_DATA_STRING_WITH_LENGTH_DO_NOT_SHOW(Buffer.from(actionData.nfts[2].hash), 0, 64),
