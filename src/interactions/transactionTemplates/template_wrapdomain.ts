import type {HexString, ParsedActionAuthorisation, ParsedTransaction, ParsedTransferDomain, ParsedWrapDomain, Uint8_t, ValidBIP32Path} from "../../types/internal"
...
const CONTRACT_ACCOUNT = parseNameString("fio.oracle", InvalidDataReason.UNEXPECTED_ERROR);
const CONTRACT_NAME = parseNameString("wrapdomain", InvalidDataReason.UNEXPECTED_ERROR);


export function template_wrapdomain(chainId: HexString, tx: ParsedTransaction, parsedPath: ValidBIP32Path): Array<Command> {
    ...
    const actionData: ParsedWrapDomain = tx.actions[0].data as ParsedWrapDomain;
    ...
    return [
        ...
        COMMAND_SHOW_MESSAGE("Action", "Wrap FIO Domain"),
        ...
        ...COMMANDS_COUNTED_SECTION([
            ...
            COMMAND_APPEND_DATA_STRING_WITH_LENGTH_SHOW("Public Address", Buffer.from(actionData.public_address)),
            COMMAND_APPEND_DATA_FIO_AMOUNT_SHOW("Max oracle fee", uint64_to_buf(actionData.max_oracle_fee).reverse()), 
