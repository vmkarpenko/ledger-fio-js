import type {HexString, ParsedActionAuthorisation, ParsedMapBlockchainPublicAddress, ParsedProxyVotesToRegisteredProxy, ParsedStakeFIO, ParsedTransaction, ParsedTransferFIOTokensData, Uint8_t, ValidBIP32Path} from "../../types/internal"
...
const CONTRACT_NAME = parseNameString("voteproxy", InvalidDataReason.UNEXPECTED_ERROR);


export function template_voteproxy(chainId: HexString, tx: ParsedTransaction, parsedPath: ValidBIP32Path): Array<Command> {
    ...
    const actionData: ParsedProxyVotesToRegisteredProxy = tx.actions[0].data as ParsedProxyVotesToRegisteredProxy;
    ...
    return [
        ...
        COMMAND_SHOW_MESSAGE("Action", "Proxy votes"),
        ...
        ...COMMANDS_COUNTED_SECTION([
            COMMAND_APPEND_DATA_STRING_WITH_LENGTH_SHOW("Proxy", Buffer.from(actionData.proxy), 3, 64),
