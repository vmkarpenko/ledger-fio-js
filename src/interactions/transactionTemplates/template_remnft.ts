import type {HexString, ParsedActionAuthorisation, ParsedMapBlockchainPublicAddress, ParsedMapNFTSignature, ParsedNFT, ParsedRemoveNFTSignature, ParsedTransaction, ParsedTransferFIOTokensData, Uint8_t, ValidBIP32Path} from "../../types/internal"
...
const CONTRACT_NAME = parseNameString("remnft", InvalidDataReason.UNEXPECTED_ERROR);
...
function template1(chainId: HexString, tx: ParsedTransaction, parsedPath: ValidBIP32Path): Array<Command> {
    ...
    const actionData: ParsedRemoveNFTSignature = tx.actions[0].data as ParsedRemoveNFTSignature;
...
export function template_remnft(chainId: HexString, tx: ParsedTransaction, parsedPath: ValidBIP32Path): Array<Command> {
