import type {HexString, ParsedActionAuthorisation, ParsedMapBlockchainPublicAddress, ParsedRemoveAllMappedAddresses, ParsedTransaction, ParsedTransferFIOTokensData, Uint8_t, ValidBIP32Path} from "../../types/internal"
...
const CONTRACT_NAME = parseNameString("remalladdr", InvalidDataReason.UNEXPECTED_ERROR);


export function template_remalladdr(chainId: HexString, tx: ParsedTransaction, parsedPath: ValidBIP32Path): Array<Command> {
    ...
    const actionData: ParsedRemoveAllMappedAddresses = tx.actions[0].data as ParsedRemoveAllMappedAddresses;
